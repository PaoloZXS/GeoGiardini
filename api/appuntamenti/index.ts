  ...
  try {
    ...
    if (req.method === 'GET') {
      ...
      try {
        ...
        res.statusCode = 200;
        res.setHeader('Content-Type', 'application/json');
        res.end(JSON.stringify({ success: true, appointments: Array.from(appointmentsMap.values()) }));
        return;
      } catch (error) {
    ...
    const selectedAttivita = Array.isArray(attivita)
      ? attivita.map((item: any) => item?.toString().trim()).filter((item: string) => item)
      : [];
    ...
    if (!trimmedData || !trimmedClienteId || selectedGiardinieri.length === 0) {
      res.statusCode = 400;
      res.setHeader('Content-Type', 'application/json');
      res.end(JSON.stringify({ success: false, message: 'Data, cliente e almeno un giardiniere sono obbligatori.' }));
      return;
    }
